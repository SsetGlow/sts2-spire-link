    ...
    public static string[] BuildPreviewLines()
    {
        ...
        return SpireLinkRegistrationBridge.AllProviders.Select(provider =>
        {
            if (provider.CardBehavior is not null)
            {
                ...
                return $"CARD {provider.BehaviorId} resonance={result.ResonanceAfter} links={result.PendingLinkCount} support={result.SupportActionCount}";
            }
            if (provider.RelicBehavior is not null)
            {
                var result = preview.PreviewRelic(provider.RelicBehavior, "player_1");
                return $"RELIC {provider.BehaviorId} resonance={result.ResonanceAfter} links={result.PendingLinkCount} support={result.SupportActionCount}";
            }
            if (provider.EventBehavior is not null)
            {
                ...
                return $"EVENT {provider.BehaviorId} resonance={result.ResonanceAfter} links={result.PendingLinkCount} support={result.SupportActionCount}";
            }
